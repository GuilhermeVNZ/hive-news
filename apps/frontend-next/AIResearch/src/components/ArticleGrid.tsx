...
const ArticleGrid = ({ selectedCategory, searchQuery }: ArticleGridProps) => {
    ...
    async function fetchArticles() {
      try {
        ...
        const featuredArticles = articles.filter(
          (a: Article) => a.featured === true,
        );
        console.log(
          `[AIResearch ArticleGrid] Fetched ${articles.length} articles, ${featuredArticles.length} featured:`,
  ...
  let byCategory = selectedCategory
    ? articles.filter(
        (a) =>
          a.imageCategories &&
          a.imageCategories.includes(selectedCategory.toLowerCase()),
      )
    : articles;
  ...
  if (!selectedCategory && words.length === 0) {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log(
        `[AIResearch ArticleGrid] Featured articles found: ${featuredArticles.length}`,
  ...
  const filteredArticles =
    words.length === 0
      ? byCategory
      : byCategory.filter((a) => {
          const topics = Array.isArray(a.imageCategories)
            ? a.imageCategories.join(" ")
            : "";
          const hay = normalize(
            `${a.title} ${a.id} ${a.excerpt} ${a.category ?? ""} ${topics}`,
          );
