...
async fn main() {
    ...
    let html_sites = vec![
        ...
        (
            "Quanta Magazine Computer Science",
...
fn print_summary(results: &[TestResult]) {
    ...
    let blocked: Vec<_> = results
        .iter()
        .filter(|r| !r.success && r.error.as_ref().map_or(false, |e| e.contains("403")))
        .collect();
    ...
    let timeouts: Vec<_> = results
        .iter()
        .filter(|r| !r.success && r.error.as_ref().map_or(false, |e| e.contains("timeout")))
        .collect();
    let need_playwright: Vec<_> = results
        .iter()
        .filter(|r| r.recommended_method == "playwright")
        .collect();
    ...
    println!("\nACTIONS REQUIRED:");
    println!(
        "  1. Update system_config.json force_js: true for {} sites",
        need_playwright.len()
    );
    println!("  2. Fix/update URLs for {} 404 sites", not_found.len());
    println!(
        "  3. Review {} other failed sites",
        failed - blocked.len() - not_found.len() - timeouts.len()
    );
