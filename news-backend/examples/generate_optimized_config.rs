...
fn main() {
    ...
    let test_results: Vec<TestResult> =
        serde_json::from_str(&results_json).expect("Failed to parse test results");

    println!("✓ Loaded {} test results\n", test_results.len());

    // Load current system_config.json
    let config_path = "../news-backend/system_config.json";
    let config_json = fs::read_to_string(config_path).expect("Failed to read system_config.json");
    let mut config: Value =
        serde_json::from_str(&config_json).expect("Failed to parse system_config.json");

    // Backup original
    let backup_path = "../news-backend/system_config.json.backup";
    fs::write(backup_path, &config_json).expect("Failed to create backup");
    ...
    if let Some(sites) = config.get_mut("sites").and_then(|s| s.as_object_mut()) {
        for (_site_name, site_config) in sites.iter_mut() {
            if let Some(collectors) = site_config
                .get_mut("collectors")
    ...
    // Save updated config
    let updated_json = serde_json::to_string_pretty(&config).expect("Failed to serialize config");
    fs::write(config_path, updated_json).expect("Failed to write system_config.json");
...
fn update_collector(
    collector: &mut serde_json::Map<String, Value>,
...
fn print_stats(stats: &Stats) {
    ...
    println!(
        "   {} sites now using RSS (faster & more reliable)",
        stats.html_to_rss
    );
    println!(
        "   {} sites configured for Playwright (for JS-heavy sites)",
        stats.added_playwright
    );
    println!(
        "   {} broken sites disabled (won't waste time collecting)",
        stats.disabled_broken
    );
