...
async fn test_full_pipeline(client: &Client, name: &str, url: &str, id: &str) -> PipelineTestResult {
    let base_domain = extract_base_domain(url);
    
    // Step 1: Check for RSS feed
    let (rss_url, rss_works, article_urls) = test_rss_and_extract_articles(client, &base_domain).await;
...
async fn test_rss_and_extract_articles(client: &Client, base_domain: &str) -> (Option<String>, bool, Vec<String>) {
...
async fn test_article_accessibility(client: &Client, article_urls: &[String]) -> (bool, f64, Vec<String>) {
    if article_urls.is_empty() {
            ...
            Ok(response) => {
                ...
                if status.is_success() {
                    if let Ok(content) = response.text().await {
                        ...
                        if content.contains("cookie") && content.contains("accept") || content.contains("gdpr") {
                            if !blocked_by.contains(&"cookies".to_string()) {
                                blocked_by.push("cookies".to_string());
                            }
                        } else if content.contains("paywall") || content.contains("subscribe") && content.len() < 5000 {
        ...
        Ok(response) => {
            ...
            match response.text().await {
                Ok(content) => {
                    ...
                    // Check for blockers
                    let has_cookie_wall = (content.contains("cookie") || content.contains("Cookie")) 
                        && (content.contains("accept") || content.contains("Accept"))
                        && content.contains("button");
                    
                    let has_paywall = (content.contains("paywall") || content.contains("subscribe") || content.contains("Subscribe"))
                        && content.len() < 10000; // Short page = likely just paywall
                    
                    let has_captcha = content.contains("captcha") || content.contains("recaptcha") || content.contains("hCaptcha");
                    
                    let has_js_only = content.contains("JavaScript") && content.contains("enable") && content.len() < 5000;
                    ...
                    // Track blockers
                    if has_cookie_wall { blocked_by.push("cookies".to_string()); }
                    if has_paywall { blocked_by.push("paywall".to_string()); }
                    if has_captcha { blocked_by.push("captcha".to_string()); }
                    if has_js_only { blocked_by.push("js_required".to_string()); }
                    if !has_content { blocked_by.push("no_content".to_string()); }
    ...
    for sel_str in content_selectors {
        if let Ok(selector) = Selector::parse(sel_str) {
            for element in document.select(&selector) {
                ...
                if text.len() > 500 { // Reasonable article length
                    return true;
                }
            }
        }
...
fn decide_recommendation(
    ...
    if rss_works && articles_accessible && article_access_rate >= 0.5 {
        return (
            ...
            false,
            format!("✓ RSS + {}% articles accessible", (article_access_rate * 100.0) as i32),
        );
    }
...
fn print_pipeline_result(result: &PipelineTestResult) {
    let status = if result.recommended_type == "rss" {
        format!("✓ RSS ({}% articles OK)", (result.article_access_rate * 100.0) as i32)
    ...
    if result.rss_feed_works {
        println!("      RSS feed: {} ({} articles found)", 
            result.rss_feed_url.as_ref().unwrap(), result.rss_article_count);
    }
...
fn print_pipeline_summary(results: &[PipelineTestResult]) {
    ...
    let rss_working: Vec<_> = results.iter().filter(|r| r.recommended_type == "rss").collect();
    let rss_playwright: Vec<_> = results.iter().filter(|r| r.recommended_type == "rss+playwright").collect();
    let html_working: Vec<_> = results.iter().filter(|r| r.recommended_type == "html").collect();
    let need_playwright: Vec<_> = results.iter().filter(|r| r.recommended_type == "playwright").collect();

    println!("✅ RSS with accessible articles: {} sites", rss_working.len());
    for r in &rss_working {
        println!("   • {} ({}% accessible)", r.name, (r.article_access_rate * 100.0) as i32);
    }

    if !rss_playwright.is_empty() {
        println!("\n⚠️  RSS exists but articles blocked: {} sites", rss_playwright.len());
    ...
    if !html_working.is_empty() {
        println!("\n✓ HTML working (no RSS or RSS not better): {} sites", html_working.len());
