...
async fn main() {
    ...
    println!(
        "✓ Found {} collectors to test (excluding offline sites)\n",
        test_sites.len()
    );
...
async fn test_adaptive_strategies(
    name: &str,
    url: &str,
    collector_type: &str,
) -> AdaptiveTestResult {
        ...
        if let Some(rss_url) = autodiscover_rss(&antibot_client, url).await {
            if let Ok((accessible, has_content, _)) = test_url_full(&antibot_client, &rss_url).await
            {
                if accessible && has_content {
                    ...
                    result.recommendation = format!(
                        "Convert to RSS: {}",
                        result.rss_autodiscover_url.as_ref().unwrap()
                    );
                    return result;
                }
            }
    ...
    for alt_url in alternative_paths {
        ...
        if let Ok((accessible, has_content, _)) = test_url_full(&antibot_client, &alt_url).await {
            if accessible && has_content {
                ...
                result.recommendation = format!(
                    "Use alternative URL: {}",
                    result.alternative_url.as_ref().unwrap()
                );
                return result;
            }
        }
    ...
    if result.blockers_found.contains(&"cookies".to_string())
        && !result.blockers_found.contains(&"403".to_string())
    {
    ...
    result.needs_playwright = true;
    result.recommendation = format!(
        "All strategies failed. Blockers: {:?}. Needs Playwright.",
        result.blockers_found
    );
    result
}

async fn test_url_full(
    client: &Client,
    url: &str,
) -> Result<(bool, bool, Vec<String>), Box<dyn std::error::Error>> {
        ...
        Ok(response) => {
            ...
            match response.text().await {
                Ok(content) => {
                    ...
                    if content.contains("paywall")
                        || (content.contains("subscribe") && content.len() < 10000)
                    {
...
fn print_summary(results: &[AdaptiveTestResult]) {
    ...
    let need_pw: Vec<_> = results
        .iter()
        .filter(|r| r.needs_playwright && !r.success)
        .collect();
    let failed: Vec<_> = results
        .iter()
        .filter(|r| !r.success && !r.needs_playwright)
        .collect();

    println!("📊 OVERALL RESULTS ({} sites tested):", total);
    println!(
        "   ✅ Successfully accessible:  {:3} ({:5.1}%)",
        successful.len(),
        (successful.len() as f64 / total as f64) * 100.0
    );
    println!(
        "   ⚠️  Need Playwright:         {:3} ({:5.1}%)",
        need_pw.len(),
        (need_pw.len() as f64 / total as f64) * 100.0
    );
    println!(
        "   ✗  Failed (all strategies): {:3} ({:5.1}%)",
        failed.len(),
    ...
    for r in &successful {
        *strategy_counts
            .entry(r.working_strategy.as_str())
            .or_insert(0) += 1;
    }
    ...
    // Sites with new URLs
    let url_changes: Vec<_> = successful
        .iter()
        .filter(|r| r.working_url != r.original_url)
        .collect();
    ...
    // RSS discoveries
    let rss_discoveries: Vec<_> = successful
        .iter()
        .filter(|r| r.rss_autodiscover_works)
        .collect();
    ...
    if !rss_discoveries.is_empty() {
        ...
        for r in &rss_discoveries {
            println!(
                "   • {} → {}",
                r.name,
                r.rss_autodiscover_url.as_ref().unwrap()
            );
        }
    }
    ...
    println!(
        "   2. Convert {} sites to RSS (faster/more reliable)",
        rss_count
    );
    println!(
        "   3. Add force_js: true for {} Playwright sites",
        need_pw.len()
    );
    println!(
        "   4. Investigate {} failed sites for manual fixes",
        failed.len()
    );
