...
async fn main() {
    ...
    let rss_sites = vec![
        ...
        (
            ...
            "https://damo.alibaba.com/news/rss",
            "rss_alibaba_damo",
        ),
        (
        ...
        (
            ...
            "https://www.theverge.com/rss/group/ai/index.xml",
            "rss_the_verge_ai",
        ),
        (
            "Wired AI RSS",
            "https://www.wired.com",
            "https://www.wired.com/feed/category/science/ai/latest/rss",
            "rss_wired_ai",
        ),
        (
    ...
    let html_sites = vec![
        ...
        (
            "Inflection AI",
            "https://inflection.ai/blog/enterprise",
            "html_inflection_ai",
        ),
        (
            "Apple ML Highlights",
        ...
        (
            ...
            "html_apple_ml_research",
        ),
        (
            "Intel AI",
            "https://www.intel.com/content/www/us/en/customer-spotlight/overview.html",
            "html_intel_ai",
        ),
        (
            "AMD AI",
            "https://www.amd.com/en/developer/resources/technical-articles.html",
            "html_amd_ai",
        ),
        (
            "Stanford HAI",
        ...
        (
            "Menlo Ventures",
            "https://menlovc.com/focus-areas/ai/",
            "html_menlo_ventures",
        ),
        (
            "AIResearch News",
            "https://www.airesearch.news/",
        ...
        (
            "Robohub",
            "https://robohub.org/category/news/",
            "html_robohub",
        ),
        (
            "IEEE Tech",
            "https://www.ieee.org/advancing-technology",
            "html_ieee",
        ),
        (
            "Robot Report",
        ...
        (
            "Robotics Biz Review",
            "https://roboticsbusinessreview.com/news",
        ...
        (
            "D-Wave",
            "https://www.dwavequantum.com/company/newsroom/",
            "html_dwave",
        ),
        (
            "Quantinuum",
            "https://www.quantinuum.com/news/news",
            "html_quantinuum",
        ),
        ...
        (
            "Robotics Online",
            "https://www.automate.org/robotics/news",
            "html_robotics_org",
        ),
        (
            "ABB Robotics",
        ...
        (
            "KUKA Robotics",
            "https://www.kuka.com/en-us/press/media-news",
            "html_kuka",
        ),
        (
            "Anthropic Research",
            "https://www.anthropic.com/research",
            "html_anthropic_research",
        ),
        ...
        (
            "Xanadu Quantum",
            "https://www.xanadu.ai/blog/",
            "html_xanadu",
        ),
        (
            "Infleqtion",
            "https://infleqtion.com/blog/",
            "html_infleqtion",
        ),
        (
            "ColdQuanta",
            "https://www.coldquanta.com/news",
            "html_coldquanta",
        ),
        (
            "QCI",
            "https://www.quantumcomputinginc.com/news",
            "html_qci",
        ),
        (
            "Universal Robots",
            "https://www.universal-robots.com/blog/",
            "html_universal_robots",
        ),
        (
            "OMRON",
            "https://automation.omron.com/en/us/news/",
            "html_omron",
        ),
        (
            "Yaskawa",
            "https://www.yaskawa.com/about-us/media-center/news",
            "html_yaskawa",
        ),
        (
            "Agility Robotics",
            "https://www.agilityrobotics.com/about/press",
            "html_agility",
        ),
...
fn get_recommendation_summary(config: &SiteConfig) -> String {
    match config.recommended_type.as_str() {
        "rss" => format!(
            "✓ RSS ({})",
            if config.rss_available.as_ref().map_or(false, |r| r.works) {
                "verified"
            } else {
                "found"
            }
...
async fn test_rss_site(
    ...
    SiteConfig {
        ...
        recommended_type: if rss_result.0 {
            "rss".to_string()
        } else {
            "broken".to_string()
        },
        ...
        reason: if rss_result.0 {
            ...
            format!(
                "RSS feed broken ({})",
                rss_result.2.unwrap_or("unknown error".to_string())
            )
        },
    }
}

async fn test_html_site_with_rss_detection(
    client: &Client,
    name: &str,
    url: &str,
    id: &str,
    ...
    let (recommended_type, recommended_url, reason) = if let Some(ref rss) = rss_info {
        (
            "rss".to_string(),
            rss.url.clone(),
            format!("Found working RSS feed at {}", rss.url),
        )
    } else if html_result.0 {
        (
            "html".to_string(),
            url.to_string(),
            "HTML page accessible".to_string(),
        )
    } else if html_result.2 == Some(403) {
        (
            "playwright".to_string(),
            url.to_string(),
            "Blocked (403) - needs Playwright".to_string(),
        )
        ...
        (
            ...
            format!(
                "Failed with: {}",
                html_result.3.unwrap_or("unknown".to_string())
            ),
        )
    };
...
async fn test_url_detailed(
    client: &Client,
    url: &str,
) -> (bool, Option<usize>, Option<u16>, Option<String>) {
    match client.get(url).send().await {
        Ok(response) => {
...
fn print_intelligent_summary(configs: &[SiteConfig]) {
    ...
    let rss_recommended: Vec<_> = configs
        .iter()
        .filter(|c| c.recommended_type == "rss")
        .collect();
    let html_recommended: Vec<_> = configs
        .iter()
        .filter(|c| c.recommended_type == "html")
        .collect();
    let playwright_needed: Vec<_> = configs
        .iter()
        .filter(|c| c.recommended_type == "playwright")
        .collect();
    let broken: Vec<_> = configs
        .iter()
        .filter(|c| c.recommended_type == "broken")
        .collect();

    // Sites that found RSS when they were HTML
    let html_to_rss: Vec<_> = configs
        .iter()
    ...
    println!(
        "  ✓ RSS (best): {} ({:.1}%)",
        rss_recommended.len(),
        (rss_recommended.len() as f64 / total as f64) * 100.0
    );
    println!(
        "  ✓ HTML (good): {} ({:.1}%)",
        html_recommended.len(),
        (html_recommended.len() as f64 / total as f64) * 100.0
    );
    println!(
        "  ⚠ Playwright needed: {} ({:.1}%)",
        playwright_needed.len(),
        (playwright_needed.len() as f64 / total as f64) * 100.0
    );
    println!(
        "  ✗ Broken/404: {} ({:.1}%)",
        broken.len(),
        (broken.len() as f64 / total as f64) * 100.0
    );
    ...
    println!("\n📋 NEXT STEPS:");
    println!(
        "  1. Update {} HTML collectors to use RSS feeds",
        html_to_rss.len()
    );
    println!(
        "  2. Add force_js: true for {} Playwright sites",
        playwright_needed.len()
    );
