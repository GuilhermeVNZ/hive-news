...
impl SemanticScholarCollector {
    ...
    pub async fn fetch_recent_papers(
        ...
        let params = vec![
            ...
            (
                "fields",
                "paperId,title,authors,abstract,venue,year,url,openAccessPdf,fieldsOfStudy",
            ),
        ...
        if !response.status().is_success() {
            ...
            if status.as_u16() == 429 {
                ...
                println!(
                    "Semantic Scholar rate-limited (429). Waiting {} ms...",
                    wait_ms
                );
        ...
        println!(
            "Found {} papers in Semantic Scholar",
            search_response.data.len()
        );

        // Filtrar por campos de estudo de tecnologia
        let mut articles = Vec::new();
        let allowed_fields: Vec<String> =
            fields_of_study.iter().map(|s| s.to_lowercase()).collect();
        ...
        for paper in search_response.data {
            ...
            let title = paper
                .title
                .unwrap_or_else(|| format!("Semantic Scholar Paper {}", article_id));
            articles.push(ArticleMetadata {
                ...
                url: paper.url.unwrap_or_else(|| {
                    format!("https://www.semanticscholar.org/paper/{}", article_id)
                }),
                author: if authors_str.is_empty() {
                    None
                } else {
                    Some(authors_str)
                },
        ...
        println!(
            "✅ Filtered to {} technology papers from Semantic Scholar",
            articles.len()
        );
        if !has_key {
    ...
    pub async fn download_pdf(&self, article_id: &str, output_path: &PathBuf) -> Result<()> {
        ...
        Err(anyhow::anyhow!(
            "PDF not available for open access: {}",
            article_id
        ))
    }
}
