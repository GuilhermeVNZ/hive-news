...
pub async fn list_logs(
    ...
    let items: Vec<ArticleLogItem> = items
        .into_iter()
        .skip(offset)
        .take(limit)
        .map(|(_, it)| it)
...
pub async fn set_hidden(
    ...
    let manager = match RegistryManager::new(&registry_path) {
        ...
        Err(e) => {
            ...
            return Json(
                serde_json::json!({"success": false, "error": format!("Failed to load registry: {}", e)}),
            );
        }
    };
    ...
    eprintln!(
        "[set_hidden] Updating hidden status: article_id={}, hidden={}, fs_title={:?}",
...
pub async fn enrich_titles_from_arxiv(
    ...
    for meta in all.iter_mut() {
        ...
        if !meta.arxiv_url.contains("arxiv.org/abs/") {
            skipped += 1;
            continue;
        }
...
pub async fn set_featured(
    ...
    eprintln!(
        "[set_featured] Updating featured status: article_id={}, featured={}, fs_title={:?}",
    ...
    match manager.set_featured(&article_id_to_update, body.featured) {
        Ok(_) => {
            tracing::info!(
                "Successfully updated featured status for article: {} (fs_title: {:?})",
                article_id_to_update,
                actual_title_from_fs
            );
            Json(serde_json::json!({"success": true}))
        ...
        Err(e) => {
            tracing::error!(
                "Failed to update featured status for article {}: {}",
                article_id_to_update,
                e
            );
            Json(serde_json::json!({"success": false, "error": format!("{}", e)}))
