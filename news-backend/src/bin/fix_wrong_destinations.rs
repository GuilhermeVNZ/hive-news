...
async fn main() -> Result<()> {
    ...
    // Load registry
    let registry_content = fs::read_to_string(registry_path).await
        .context("Failed to read registry file")?;
    
    let mut registry: serde_json::Value = serde_json::from_str(&registry_content)
        .context("Failed to parse registry JSON")?;
    
    let articles = registry["articles"].as_object_mut()
    ...
    for (article_id, article_data) in articles.iter_mut() {
        // Get destinations
        let destinations = article_data["destinations"]
            .as_array()
            .and_then(|d| {
                d.iter()
        ...
        let current_output_dir = article_data["output_dir"]
            .as_str()
            .map(PathBuf::from);
        ...
        let correct_output_dir = Path::new("G:/Hive-Hub/News-main/output")
        ...
        if current_output_dir != correct_output_dir {
            ...
            if current_output_dir.exists() {
                ...
                fs::rename(&current_output_dir, &correct_output_dir).await
    ...
    if fixed_count > 0 || error_count > 0 {
        ...
        fs::copy(registry_path, &backup_path).await
            .context("Failed to create backup")?;
        println!("💾 Backup created: {}\n", backup_path);
        
        // Save updated registry
        fs::write(registry_path, updated_json).await
