...
fn main() -> Result<()> {
    ...
    let system_config_path = possible_system_paths
        .iter()
        .find(|p| p.exists())
        .ok_or_else(|| {
            anyhow::anyhow!("system_config.json not found in any of the expected locations")
        })?;

    println!(
        "✅ Found system_config.json: {}",
        system_config_path.display()
    );

    // Find or create collectors_config.json path (use first found or default)
    let collectors_config_path = possible_collectors_paths
        .iter()
        .find(|p| p.exists())
        .map(|p| p.to_path_buf())
...
fn sync_collectors_config(system_config_path: &Path, collectors_config_path: &Path) -> Result<()> {
    ...
    let system_content = fs::read_to_string(system_config_path).context(format!(
        "Failed to read system_config.json: {}",
        system_config_path.display()
    ))?;

    let system_config: Value =
        serde_json::from_str(&system_content).context("Failed to parse system_config.json")?;

    let sites = system_config
        .get("sites")
                    ...
                    if let Some(collector) = collector_value.as_object() {
                        if let Some(collector_id) = collector.get("id").and_then(|v| v.as_str()) {
                            ...
                            if !collectors_map.contains_key(collector_id) {
                                collectors_map
                                    .insert(collector_id.to_string(), collector_value.clone());
                            } else {
    ...
    let enabled_count = all_collectors
        .iter()
    ...
    fs::write(collectors_config_path, collectors_content).context(format!(
        "Failed to write collectors_config.json: {}",
        collectors_config_path.display()
    ))?;
