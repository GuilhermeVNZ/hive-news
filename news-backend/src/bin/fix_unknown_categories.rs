...
fn main() -> Result<()> {
    ...
    let downloads_base = Path::new("G:/Hive-Hub/News-main/downloads/raw");
    ...
    for entry in entries {
        ...
        if source_file.exists() {
            ...
            if current_source == "unknown" || current_source == "Other" || current_source.is_empty() {
                ...
                println!("     📝 Updated source.txt: {} → {}", current_source, correct_category);
            }
        } else {
        ...
        fs::write(&source_file_after, &correct_category)
            .context(format!("Failed to update source.txt in {:?}", new_article_dir))?;
